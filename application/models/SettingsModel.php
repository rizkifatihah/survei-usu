...
class SettingsModel extends CI_Model
{
    ...
    {
        $query = $this->db->where('id_profile', '1')->get('survei_identitas');
        return $query->row();
    }

    public function getMetodePembayaran(){
        $this->datatables->select('*,survei_payment_method.id_payment_method as id_payment_method');
        $this->datatables->from('survei_payment_method');
    ...
    public function getChannelPembayaran(){
        $this->datatables->select('*,survei_payment_channel.id_payment_channel as id_payment_channel');
        $this->datatables->from('survei_payment_channel');
        $this->datatables->add_column(
            'action',
